...
const char* ssid = "Kunal"; //Enter SSID
const char* password = "Kunnumunnu1"; //Enter Password
const char* websockets_server_host = "wskunalm.herokuapp.com"; //Enter server adress
...
void loop() {
  ...
  if (pingCount > 9) {
    ...
    if (Message == "?") {
      ...
      digitalWrite(ledPin, HIGH); //setting the pin_data=1
      client.send("\"cstate\":\"ON\"");

    } else if (Message == "CMD:off") { //if command then execute
      Serial.println("Sending CMD");
      digitalWrite(ledPin, LOW); //setting pin_data=0
